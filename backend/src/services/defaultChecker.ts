 ...
 * Fetches IDs of loans that are overdue based on the current ledger sequence.
 *
 * A loan becomes overdue only after the current ledger has passed its due
 * ledger. Loans whose due ledger is equal to the current ledger are still
 * considered due and must not be returned.
 */
private async fetchOverdueLoanIds(currentLedger: number): Promise<number[]> {
  const result = await query(
    `
